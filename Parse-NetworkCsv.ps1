...
function Resolve-NetworkAssetId {
    ...
    .SYNOPSIS
        ...
        using the group member-candidates endpoint.

    .DESCRIPTION
        Searches /groups/custom/member-candidates?_search=<CIDR>&_limit=100&_offset=0
        The CIDR is URL-encoded so the "/" becomes %2F.
    ...
    try {
        ...
        if ($results.items -and @($results.items).Count -gt 0) {
            # Match the exact CIDR in the results to avoid partial matches
            # e.g., searching "10.20.51.0/24" should not pick up "10.20.51.0/25"
            foreach ($item in @($results.items)) {
                # Check if the item name or description contains the exact CIDR
            ...
            # (the API search is typically precise for CIDR notation)
